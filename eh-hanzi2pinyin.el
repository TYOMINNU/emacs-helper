...
(defvar hanzi-pinyin-table (make-hash-table))
(setq hanzi-pinyin-map 
      '(("a" "阿啊呵腌嗄锕吖")
...
(defun eh-make-hanzi-pinyin-table (hanzi-pinyin-map)
  (loop for (pinyin hanzi-string) in map
        do (loop for hanzi across hanzi-string
                 do (let ((existing (gethash hanzi pinyin-hanzi-table)))
                      (puthash hanzi
...
(defun eh-hanzi2pinyin (string)
  (apply #'concat
         (loop for c across string
               collecting (concat "" (or (gethash c hanzi-pinyin-table)
                                         c)))))
